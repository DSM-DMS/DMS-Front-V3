...
const GoingoutCardContainer = (props) => {
    const { goingoutCard, onChangeCard, cardList } = props;
    const goingoutCards = cardList.map((card) => {
        ...
        return (<GoingoutCard gooutDate = {card.goOutDate} returnDate = {card.returnDate} reason = {card.reason} key = {card.id} selectedClass = {selectedClass} onChangeCard = {onChangeCard} id = {card.id}/>);
    });
...
export default connect(mapStateToProps, mapDispatchToProps)(GoingoutCardContainer);