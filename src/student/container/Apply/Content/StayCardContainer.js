...
const StayCardContainer = ({selectedType, onChangeType}) => {
    const cardInfoList = [
        {
            type: '금요귀가',
    ...
    const cardList = cardInfoList.map((info, i) => {
        ...
        return (<StayCard content = {info.content} title = {info.type} iconSrc = {info.iconSrc} onChangeType = {onChangeType} selectedClass = {selectedClass} key = {i}/>)
    })
